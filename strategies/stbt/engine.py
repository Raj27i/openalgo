...
if UNDERLYING not in INDEX_MAP:
    raise SystemExit(
        f"[STBT] Unsupported underlying {UNDERLYING!r} — supported: {sorted(INDEX_MAP)}"
    )
...
def _notify(text: str):
    ...
    def _send():
        try:
            ...
            telegram_alert_service.send_alert_sync(chat_id, f"[{UNDERLYING} STBT] {text}")
        except Exception as exc:
...
def _smart_flatten(symbol: str, label: str, critical: bool = False) -> tuple[str | None, str]:
    ...
    Idempotent: if the broker already shows the position flat, no order is
    placed — a blind BUY-to-close can never open a wrong-direction position
    (the core reason for using smart order on exits). Returns (orderid, outcome):
      (oid,  "placed") an order was actually sent — price the exit off it
      (None, "flat")   broker already flat ("no action needed") — drift-recovered
      (None, "failed") could not flatten after all retries — MANUAL ACTION

    CRITICAL: we send quantity=0 as well as position_size=0. The broker algo's
    degenerate branch (position_size==0 AND current==0) would otherwise place a
    BUY of the request quantity — opening a long. quantity=0 makes that branch a
    true no-op; in the real short→0 case the request quantity is ignored anyway
    (the broker derives abs(current))."""
    ...
    for attempt in range(1, retries + 1):
        try:
            resp = client.placesmartorder(
                strategy=STRATEGY,
                symbol=symbol,
                action="BUY",  # required field; real side derived from position sign
                exchange=OPT_EXCHANGE,
                price_type="MARKET",
                product=PRODUCT,
                quantity=0,
            ...
            if isinstance(resp, dict) and resp.get("status") == "success":
                ...
                # success but no orderid and not a "matched" message — treat as
                # placed-unknown so the caller prices the exit off LTP.
                return None, "placed"
            log.warning(
...
def find_hedge_strike(chain: dict, opt_type: str) -> tuple[str | None, float | None]:
    ...
    if best_sym is None:
        log.warning(
            "[HEDGE] No OTM %s strike found for ₹%.0f target.", opt_type, HEDGE_TARGET_PREMIUM
        )
...
def _set_phase(
    phase: str, main_legs: list | None = None, hedge=None, expiry: str = "", message: str = ""
):
...
def _kill_session(main_legs: list, hedge, expiry: str, save_cb, total: float) -> None:
    ...
    log.critical(
        "[KILL] Session net %.2f breached max loss ₹%.0f — force-exiting ALL positions.",
        total,
        MAX_LOSS,
    )
    _notify(
    ...
    if not open_legs and not hedge_open:
        ...
        save_cb()
        log.critical(
            "[KILL] *** Some exits FAILED — positions remain at broker. MANUAL ACTION REQUIRED. ***"
        )
        _notify(
            "KILL SWITCH WARNING: some exits FAILED — check broker "
...
_hist_candle_cache: dict[
    str, tuple[str, str, float]
] = {}  # symbol -> (poll_min, candle_min, close)
...
def get_last_closed_1m_candle(symbol: str) -> tuple[str, float] | None:
    ...
    try:
        ...
        log.warning(
            "[REENTRY-1M] history() failed for %s: %s — falling back to WS candle.", symbol, exc
        )
        result = None
    ...
    if result:
        log.info(
            "[REENTRY-1M] %s last-closed 1m candle %s close=%.2f", symbol, result[0], result[1]
        )
    return result
...
class MainLeg:
    ...
    def _log_cycle_pnl(self, exit_price: float, reason: str):
        ...
        log.info(
            "[%s CYCLE %d %s] entry=%.2f  exit=%.2f  qty=%d  "
    ...
    def _try_entry(self, ltp: float, save_cb) -> bool:
        ...
        if drop < ENTRY_DROP_PCT:
            return False
        log.info(
            "[%s] Entry trigger: %.2f%% drop  ref=%.2f  ltp=%.2f",
            self.opt_type,
            drop,
            self.ref_premium,
            ltp,
    ...
    def _try_reentry(self, ltp: float, save_cb, source: str = "LTP") -> bool:
        ...
        if ltp > self.entry_price:
            return False
        log.info(
            "[%s] Re-entry trigger (%s): price=%.2f ≤ entry=%.2f",
            self.opt_type,
            source,
            ltp,
            self.entry_price,
    ...
    def _sell(self, ref_ltp: float, save_cb) -> bool:
        ...
        log.info(
            "[%s] SHORT entered  entry=%.2f  sl=%.2f  qty=%d",
            self.opt_type,
            filled,
            self.sl_price,
            self.quantity,
        ...
        _notify(
            f"SHORT {self.opt_type} {self.symbol} @ ₹{filled:.2f} "
    ...
    def _check_sl(self, ltp: float, save_cb) -> bool:
        if ltp < self.sl_price:
            return False
        log.warning("[%s] SL HIT  ltp=%.2f ≥ sl=%.2f", self.opt_type, ltp, self.sl_price)

        if USE_SMART_EXIT:
            oid, outcome = _smart_flatten(self.symbol, f"{self.opt_type}_SL", critical=False)
            if outcome == "failed":
                log.error("[%s] SL flatten failed — retrying next tick.", self.opt_type)
                return False
            if outcome == "flat":
                # Broker already flat while we thought short — drift recovered.
                # Do NOT re-enter from an uncertain state; close this leg.
                log.warning(
                    "[%s] SL: position already flat (drift recovered) — marking DONE, no re-entry.",
                    self.opt_type,
                )
                self.state = self.DONE
                _notify(
                    f"SL {self.opt_type} {self.symbol}: broker already flat "
                    "(drift recovered) — leg closed, P&L for this cover unknown."
                )
                save_cb()
                return True
        ...
        log.info(
            "[%s] Short covered.  Re-entries remaining: %d",
            self.opt_type,
            MAX_REENTRIES - self.reentries,
        )
        _notify(
            f"SL HIT {self.opt_type} {self.symbol}: covered @ ₹{exit_price:.2f} "
    ...
    def is_short(self) -> bool:
        return self.state == self.IN_SHORT
...
class HedgeLeg:
    ...
    def __init__(self, symbol: str, opt_type: str, buy_price: float = 0.0, quantity: int = 0):
        self.symbol = symbol
        self.opt_type = opt_type
        self.quantity = quantity
    ...
    def from_dict(cls, d: dict) -> "HedgeLeg":
        h = cls(d["symbol"], d["opt_type"], d.get("buy_price", 0.0), d.get("quantity", 0))
        h.state = d["state"]
    ...
    def exit_at_market(self, save_cb) -> bool:
        if self.state != self.OPEN:
            return True
        log.info(
            "[HEDGE-%s] Selling at market  symbol=%s  bought@%.2f",
            self.opt_type,
            self.symbol,
            self.buy_price,
        )
        ...
        if USE_SMART_EXIT:
            oid, outcome = _smart_flatten(self.symbol, f"HEDGE-{self.opt_type}", critical=True)
            if outcome == "flat":
                # Hedge already flat — drift recovered. No sale price to book.
                self.state = self.DONE
                log.warning(
                    "[HEDGE-%s] Already flat (drift recovered) — no sale booked.", self.opt_type
                )
                _notify(
                    f"Hedge {self.symbol}: broker already flat (drift recovered) — "
                    "closed, sale P&L unknown."
                )
                save_cb()
                return True
            if outcome == "failed":
                save_cb()
                return False
            exit_price = _get_fill_price(oid, self.symbol, live_ltp(self.symbol) or self.buy_price)
        else:
        ...
        self.charges_total = charges["total"]
        log.info(
            "[HEDGE-%s CYCLE] buy=%.2f  sell=%.2f  qty=%d  Gross=%s%.2f  Charges=%.2f  Net=%s%.2f",
            self.opt_type,
            self.buy_price,
            exit_price,
            self.quantity,
        ...
        self.state = self.DONE
        _journal_cycle(
            "HEDGE", 1, "HEDGE", self.buy_price, exit_price, self.quantity, cycle_pnl, charges
        )
...
def _log_pnl_summary(main_legs: list, hedge, label: str):
    ...
    if hedge:
        charges = getattr(hedge, "charges_total", 0.0)
        parts.append(
            f"HEDGE-{hedge.opt_type} gross={hedge.realized_pnl:+.2f} charges={charges:.2f}"
        )
        gross_total += hedge.realized_pnl
        charges_total += charges
    net_total = gross_total - charges_total
    log.info(
        "[%s PnL] %s  ->  Gross=%+.2f  Charges=%.2f  Net=%+.2f",
        label,
        "  ".join(parts),
        gross_total,
        charges_total,
...
def run_day1():
    ...
    log.info("SENSEX LTP=%.2f  ATM=%s", chain.get("underlying_ltp"), chain.get("atm_strike"))
    ...
    log.info("[WS] Feed live. Monitoring for %.0f%% entry trigger...", ENTRY_DROP_PCT)
        ...
        for leg in main_legs:
            if not leg.is_done and not _shutdown.is_set():
                ...
                if ltp > 0:
                    ...
                    if (
                        REENTRY_METHOD.upper() == "CANDLE_CLOSE"
                        and leg.state == MainLeg.SL_HIT
                        and leg.reentries < MAX_REENTRIES
                    ):
                        closed = (
                            get_last_closed_1m_candle(leg.symbol)
                            if REENTRY_CANDLE_SOURCE.upper() == "HISTORY"
                            else None
                        )
                        if closed is None:
    ...
    if not _shutdown.is_set():
        ...
        if len(open_shorts) == 1:
            naked = open_shorts[0]
            log.info("-" * 60)
            log.info("03:26 PM  %s leg is NAKED — placing overnight hedge.", naked.opt_type)
            log.info("-" * 60)
            ...
            if h_sym:
                log.info(
                    "[HEDGE] BUY %s  ltp≈₹%.2f  (target ₹%.0f)", h_sym, h_ltp, HEDGE_TARGET_PREMIUM
                )
                oid = _place_order(h_sym, "BUY", quantity, reason=f"HEDGE_{naked.opt_type}_BUY")
                if oid:
                    fill_price = _get_fill_price(oid, h_sym, h_ltp or 0.0)
                    hedge = HedgeLeg(h_sym, naked.opt_type, buy_price=fill_price, quantity=quantity)
                    log.info("[HEDGE-%s] Placed: %s  paid≈₹%.2f", naked.opt_type, h_sym, fill_price)
                    _save()
                    _set_phase(
                        "HEDGED",
                        main_legs,
                    ...
                    _notify(
                        f"Overnight hedge bought: {h_sym} @ ₹{fill_price:.2f} "
    ...
    if has_open_shorts or has_hedge:
        _save()
        log.info(
            "[STATE] Persisted for Day-2 (shorts=%d  hedge=%s)",
        ...
        log.info("[STATE] No open positions — skipping save. Next run will go to Day-1.")
        # Session ended flat on Day-1 itself — record it now (no Day-2 run).
        _append_history("DONE", main_legs, hedge, expiry)
        _notify(f"Day-1 closed flat — session over. Net P&L: ₹{_total_net(main_legs, hedge):,.2f}")
...
def run_day2():
    ...
    if not active:
        ...
        if hedge is None or hedge.is_done:
            ...
            log.critical(
                "[DAY-2] *** Hedge %s still OPEN (sell failed). "
    ...
    for leg in active:
        log.info(
            "[%s] Loaded state=%s  entry=%.2f  sl=%.2f  re-entries_used=%d",
            leg.opt_type,
            leg.state,
            leg.entry_price,
            leg.sl_price,
    ...
    while not _shutdown.is_set():
        ...
        if now_ts - _last_status_log >= 120:
            log.info(
                "  ".join(
                    f"{leg.opt_type} {live_ltp(leg.symbol):.2f}[{leg.state}]" for leg in active
                )
            )
            _last_status_log = now_ts
    ...
    if not open_legs and not hedge_open:
        ...
        _notify(
            "CRITICAL: Day-2 exits incomplete — open positions remain at the "
...
def _state_has_work(state: dict | None) -> bool:
    ...
    if not state:
        return False
    has_short = any(leg.get("state") == MainLeg.IN_SHORT for leg in state.get("main_legs", []))
    has_reentry_pending = any(
        leg.get("state") == MainLeg.SL_HIT and int(leg.get("reentries", 0)) < MAX_REENTRIES
...
def main():
    ...
    log.info("=" * 60)
    log.info(
        "%s STBT engine v%s  |  id=%s  |  lots=%d  |  hedge≈₹%.0f  |  host=%s",
        UNDERLYING,
        VERSION,
        STRATEGY_ID,
        LOT_MULTIPLIER,
    ...
    try:
        ...
        if state and not _state_has_work(state):
            log.warning(
                "[STATE] Loaded state has no open shorts or hedge — "
        ...
        _notify(
            f"ENGINE ERROR: {exc} — check the strategy log. If positions are "
